...
module.exports = function* (opts, conf) {
  ...
  if (conf._event) {
    child
      .on('error', d => conf._event.emit('start_error', d.toString()))
      .once('exit', d => conf._event.emit('exit', d.toString()))
      .once('exit', launched)
    child.stdout.on('data', d => conf._event.emit('log_data', d.toString('utf8')))
    child.stderr.on('data', d => conf._event.emit('log_error', d.toString('utf8')))
  }
  return child
}
