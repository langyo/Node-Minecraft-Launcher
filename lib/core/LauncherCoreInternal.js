...
module.exports=function(core,options){
    var event=new EventEmitter(),args=new MinecraftLaunchArguments();
    process.nextTick(()=>GenerateArguments(core,options,args,event,(re,libs)=>{
        if(re){
            if(re=='MissLibrary'&&libs.length!=0)event.emit('miss',libs);
            event.emit('error',re);
            return false;
        }
        ...
        try{
            ...
            options.Version.Natives.forEach(n=>{
                ...
                if(fs.existsSync(l))UnzipFile(l,args.NativePath,n.Options);else{
                    ...
                    if(!n.Url)missLibrary.push(name);else missLibrary.push({Name:name,Url:n.Url});
                }
            });
