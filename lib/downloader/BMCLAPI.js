    ...
    DownloadLibraries(libs,p,fun){
        ...
        for(var i=0;i<libs.length;i++){
            ...
            try{
                ...
                if(libs[i].Url==undefined)arr.push({Url:'http://bmclapi2.bangbang93.com/libraries/'+libs[i],File:file});
                else if(libs[i].Url==='http://files.minecraftforge.net/maven/'){
                    if(libs[i].Name.indexOf('net/minecraftforge/forge/')==-1)arr.push({Url:'http://bmclapi2.bangbang93.com/libraries/'+libs[i].Name,File:file});
                    else arr.push({Url:'http://bmclapi2.bangbang93.com/libraries/'+libs[i].Name.replace('.jar','-universal.jar'),File:file});
                }else if(libs[i].Url.indexOf(libs[i].Name)==-1)arr.push({Url:libs[i].Url+libs[i].Name,File:file});else arr.push({Url:libs[i].Url,File:file});
            }
        }
