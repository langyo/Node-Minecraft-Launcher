...
module.exports=function(email,password){
    ...
    this.AsyncDo=function(fun){
        ...
        if(isNull(this.AccessToken)||isNull(this.UUID)||isNull(this.DisplayName))post(`{"agent":{"name":"Minecraft","version":1},"username":"${email}","password":"${password}","clientToken":"${this.ClientToken}"}`,re=>{
            var j=JSON.parse(re);
    ...
    this.Signout=function(fun){
        ...
        post(`{"username":"${email}","password":"${password}"}`,d=>fun(!d),'signout');
    }
    this.Invalidate=function(fun){
        if(v())throw('AccessToken or ClientToken are null');
        post(`{"accessToken":"${this.AccessToken}","clientToken":"${this.ClientToken}"}`,d=>fun(!d),'invalidate');
    }
    function v(){
        return !this.AccessToken||!this.ClientToken;
    }
    this.toString=function(){
        if(!this.AccessToken||!this.UUID||!this.DisplayName)return false;
