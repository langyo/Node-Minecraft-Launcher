    ...
    return {
        ...
        Do: function*() {
            ...
            if (!this.AccessToken || !this.UUID || !this.DisplayName) {
                var re = yield fetch('https://authserver.mojang.com/authenticate', {
                    method: 'POST',
                    body: `{"agent":{"name":"Minecraft","version":1},"username":"${email}","password":"${password}","clientToken":"${this.ClientToken}"}`
                });
                re = yield re.json();
